...
```bash
php artisan make:class Services/AgentManager --no-interaction
```
...
```bash
php artisan make:interface Contracts/AgentDriver --no-interaction
```
...
```bash
php artisan make:class Drivers/ClaudeDriver --no-interaction
```
...
```bash
php artisan make:job RunAgentJob --no-interaction
```
...
**Components:**
- `resources/js/Components/AgentTerminal.tsx` - Terminal output display with syntax highlighting
- `resources/js/Components/AgentControls.tsx` - Start/stop buttons, model selector
- `resources/js/Components/CommitList.tsx` - Show commits made by agent
...
Echo.channel(`task.${taskId}`)
    .listen('AgentOutputReceived', (e) => {
        appendToTerminal(e.line)
    })
    .listen('AgentStatusChanged', (e) => {
        updateTaskStatus(e.status)
    })
```
