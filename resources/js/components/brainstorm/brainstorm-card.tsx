        ...
        <Link href={show.url([brainstorm.project_id, brainstorm.id])}>
            <Card className='p-4 hover:bg-accent transition-colors cursor-pointer'>
                <div className='space-y-3'>
                    ...
                    {brainstorm.user_context && <p className='text-sm text-muted-foreground line-clamp-2'>{brainstorm.user_context}</p>}

                    {brainstorm.status === 'completed' && brainstorm.ideas && (
                        <p className='text-sm font-medium'>
                            {brainstorm.ideas.length} {brainstorm.ideas.length === 1 ? 'idea' : 'ideas'} generated
                        </p>
                    )}

                    {brainstorm.status === 'failed' && brainstorm.error_message && <p className='text-sm text-red-600'>{brainstorm.error_message}</p>}
                </div>
            </Card>
        </Link>
    );
